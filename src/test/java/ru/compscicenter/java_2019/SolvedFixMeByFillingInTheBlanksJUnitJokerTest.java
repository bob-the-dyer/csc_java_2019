...
public class SolvedFixMeByFillingInTheBlanksJUnitJokerTest {
    ...
    public void _1_whoIsTheOutsider() {
        ...
        assertEquals("lindy", horses[0].name);
    ...
    public void _3_whatStringIsConstructed() {
        ...
        assertEquals("pipupa", new B().someString);
    }
    ...
    public void _5_methodPreferences() {
        ...
        assertEquals("int", w.whoAmI(1));

        assertEquals("Integer", w.whoAmI(Integer.valueOf(1)));

        long l = 1;
        assertEquals("Object", w.whoAmI(l));

        Long L = 1L;
        assertEquals("Object", w.whoAmI(L));

        byte b = 1;
        assertEquals("int", w.whoAmI(b));

        Double D = 1D;
        assertEquals("Object", w.whoAmI(D));
